...
struct SortedRange{T,P<:AbstractRange{T},O} <: AbstractRange{T}
    ...
    function SortedRange{T,P,O}(p::P, ::NotOrderedTrait) where {T,P,O}
    ...
    SortedRange{T,P,O}(p::P, ::IsOrderedTrait) where {T,P,O} = new{T,P,O}(p)
end
...
function SortedRange(vec_ord::Tuple{AbstractVector,Ordering})
    return SortedRange(first(vec_ord), last(vec_ord), IsOrdered)
end

SortedRange(v::AbstractVector) = SortedRange(v, order(v), IsOrdered)
function SortedRange(v::AbstractVector, o::Ordering, sorted_state::SortedTrait=NotOrdered)
...
Base.reverse(sv::ReverseRange) = SortedRange(reverse(parent(sv)), Forward, IsOrdered)
Base.reverse(sv::ForwardRange) = SortedRange(reverse(parent(sv)), Reverse, IsOrdered)
